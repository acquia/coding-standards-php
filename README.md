...
1. Optionally [configure PHP Code Sniffer integration in PhpStorm](https://www.jetbrains.com/help/phpstorm/using-php-code-sniffer.html) or your IDE or code editor of choice. You can import [`example/PhpStormInspections.xml`](example/PhpStormInspections.xml) to set up this integration automatically. To set it up manually, follow the instructions in that file’s comments. In addition to setting up PHP Code Sniffer integration, this will also enable and disable certain built-in PhpStorm inspections to supplement and deconflict those provided by PHP Code Sniffer.
